...
<head> @include('admin.partials.css') </head>

<body id="page-top"> {{-- id="page-top" --}}
    <!-- Page Wrapper -->
    <div id="wrapper">
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            <!-- Main Content -->
            <div id="content">
                <!-- Topbar --> @include('admin.partials.topbar')
                <!-- End of Topbar -->
                        ...
                        <div class="flex flex-col items-center justify-center px-3 py-3 mx-auto md:h-screen lg:py-0">
                            <div
                                ...
                                <div class="p-6 space-y-4 md:space-y-2 sm:p-8">
                                    <form class="space-y-4 md:space-y-6" action="" method="post"> @csrf @method('PATCH')
                                        <div>
                                            <div> <label for="title"
                                                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                                                    Select Category: </label>
                                                <div class="relative"> <select name="category" required
                                                        ...
                                                        {{-- @foreach ($categories as $category) <option
                                                            value="{{ $category->id }}" {{ $question->category_id ==
                                                            $category->id ? 'selected' : '' }}> {{ $category->name }}
                                                        </option> @endforeach --}} </select> </div>
                                            </div>
                                            <div> @foreach ($questions as $question ) <label for="title"
                                                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                                                    Question Name </label> <input value="{{ $question->title }}"
                                                    type="text" name="title"
                                                    class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                                    placeholder="Question Title" required> @error('title') <p
                                                    class="mb-4 text-red-600">{{ $message }}</p> @enderror @endforeach
                                            </div>
                                        </div>
...
<!-- Footer --> @include('admin.partials.footer')
<!-- End of Footer -->
</div> <!-- End of Content Wrapper -->
</div> <!-- End of Page Wrapper --> @include('admin.partials.script') </body>
